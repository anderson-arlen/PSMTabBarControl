//
//  PSMUpdatedMetalTabStyle.m
//  PSMTabBarControl
//
//  Created by John Pannell on 2/17/06.
//  Copyright 2006 Positive Spin Media. All rights reserved.
//
// This is basically a copy of PSMMetaTabStyle with a few changes

#import "PSMUpdatedMetalTabStyle.h"
...
@implementation PSMUpdatedMetalTabStyle
...
- (void)drawTabCell:(PSMTabBarCell *)cell {
	...
	NSBezierPath* hiddenLine = nil;
	float radius = 5.0;
	
	//lineColor = [NSColor darkGrayColor];
	lineColor = [NSColor colorWithCalibratedWhite:(140.0f / 255.0f) alpha:1.0];
	fillColor = [NSColor colorWithCalibratedWhite:(205.0f / 255.0f) alpha:1];
	//fillColor = [NSColor colorWithCalibratedRed:205.0/255.0 green:205.0/255.0 blue:205.0/255.0 alpha:1.0];
	
	[NSGraphicsContext saveGraphicsState];
	[[NSGraphicsContext currentContext] setShouldAntialias:YES]; // why not anti-alias???
	...
	if([cell state] == NSOnState) {
		...
		if(orientation == PSMTabBarHorizontalOrientation) {
			...
			[bezier lineToPoint:NSMakePoint(aRect.origin.x, aRect.origin.y+aRect.size.height-radius)];
			[bezier appendBezierPathWithArcFromPoint:NSMakePoint(aRect.origin.x, aRect.origin.y+aRect.size.height)
											 toPoint:NSMakePoint(aRect.origin.x+radius, aRect.origin.y+aRect.size.height)
											  radius:radius];
			[bezier lineToPoint:NSMakePoint(aRect.origin.x+aRect.size.width-radius, aRect.origin.y+aRect.size.height)];
			
			[bezier appendBezierPathWithArcFromPoint:NSMakePoint(aRect.origin.x+aRect.size.width, aRect.origin.y+aRect.size.height)
											 toPoint:NSMakePoint(aRect.origin.x+aRect.size.width, aRect.origin.y+aRect.size.height-radius)
											  radius:radius];
			[bezier lineToPoint:NSMakePoint(aRect.origin.x+aRect.size.width, aRect.origin.y)];
			
			if ( [[cell controlView] frame].size.height < 2) {
				// special case of hidden tab bar; need line across top of cell
				hiddenLine = [NSBezierPath bezierPath];
				[hiddenLine moveToPoint:NSMakePoint(aRect.origin.x, aRect.origin.y+0.5)];
				[hiddenLine lineToPoint:NSMakePoint(aRect.origin.x+aRect.size.width, aRect.origin.y+0.5)];
			}
...
- (void)drawBackgroundInRect:(NSRect)rect {
	...
	[[NSColor colorWithCalibratedWhite:(180.0f / 255.0f) alpha:1.0] set];
	NSRectFillUsingOperation(rect, NSCompositeSourceAtop);
	//[[NSColor darkGrayColor] set];
	[[NSColor colorWithCalibratedWhite:(140.0f / 255.0f) alpha:1.0] set];
